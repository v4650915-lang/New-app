...
M1`;
    ...
    if (currentMachine === 'fms') {
        const toolNum = toolCounter; // e.g. 1
        // FMS turning cycle G164
        // Q: depth of cut, D: retraction, F: feed, A, B: finish allowance, P: finish feed
        return `LIMS600
rem(T${toolNum} ${comment.replace(/[()]/g, '')})
G96S200M03F0.25
T${toolNum}
G0X60Z2
G164Q2D0.5F0.25G0A0.1B0.1P0.1
G0X20Z2
G1Z0
X50Z-30
endcont
...
M1`;
    ...
    if (currentMachine === 'fms') {
        // FMS facing cycle G165
        const toolNum = toolCounter;
        return `LIMS600
rem(T${toolNum} TORCOVKA)
G96S200M03F0.25
T${toolNum}
G0X60Z2
G165Q0.3D0.5G0A0.1B0.1F0.2P0.1
G1Z-2.0
X-1.6
Z1.0
endcont
...
M1`;
    ...
    if (currentMachine === 'fms') {
        // FMS Threading G162
        const toolNum = toolCounter;
        return `rem(T${toolNum} REZBA)
G97M3S500
T${toolNum}
G0X19Z4
G162X16Z-22H2000P0.9Q0.15A5F2.0E5G1
endcont
G0X150Z100
M1`;
    }
...
M1`;
    ...
    if (currentMachine === 'fms') {
        ...
        if (subtype === 'face') {
            // G159 Face Grooving
            return `rem(T${toolNum} KANAVKA TOR)
G97S500M3
T${toolNum}
G0X50Z2
G159X35Z-5Q3000P1000F0.05
G0X100Z100
M1`;
        } else {
            // G158 External Grooving
            return `rem(T${toolNum} KANAVKA NAR)
G97S500M3
T${toolNum}
G0X55Z-10
G158X30Z-15Q3000P0F0.1
G0X100Z100
M1`;
        }
    }
...
function resetCode() {
    ...
    if (currentMachine === 'fanuc') {
        textarea.value = TEMPLATE_HEADER_FANUC + "\n%";
        frameCounter = 1;
        toolCounter = 1;
        historyStack = [];
    } else if (currentMachine === 'fms') {
        textarea.value = TEMPLATE_HEADER_FMS;
        frameCounter = 1;
        toolCounter = 1;
        historyStack = [];
...
function addDrilling() {
    if (currentMachine === 'fanuc' || currentMachine === 'fms' || currentMachine === 'haas') {
        ...
        if (currentMachine === 'fms') {
            // FMS Drilling G154
            const toolNum = toolCounter;
            const code = `rem(T${toolNum} SVERLENIE)
G97M3S1000
T${toolNum}
G0X0Z2
G154Z-30Q5000F0.1
...
function addCutoff() {
    if (currentMachine === 'fanuc' || currentMachine === 'fms' || currentMachine === 'haas') {
        const toolCode = getNextToolCode();
        if (currentMachine === 'fms') {
            // FMS often uses same cycle logic or simple G1, use G158 as groove or just G75 equivalent? 
            // Based on files, FMS uses G cycles. Let's assume standard programming or G158 for cutoff.
            const toolNum = toolCounter;
            const code = `rem(T${toolNum} OTREZKA)
G97S800M3
T${toolNum}
G0X55Z-30
G158X0Z-30Q1000P0F0.08`;
            appendBlock(code);
        } else {
            const code = `G28U0
G28W0
...
function copyCode() {
    ...
    try {
        navigator.clipboard.writeText(textarea.value).then(() => {
            showCustomAlert('Скопировано! Не забудь изменить значения в программе на свои!');
        });
    } catch (err) {
        // Fallback
...
async function shareCode() {
    ...
    if (!window.isSecureContext) {
        copyCode();
        showCustomAlert("⚠️ Функция 'Поделиться' работает только на защищенном сайте (HTTPS).\n\nСейчас текст просто скопирован. После публикации в Телеграм кнопка заработает полноценно!");
        return;
    }

    const fileName = (currentMachine === 'fanuc' || currentMachine === 'fms' || currentMachine === 'haas') ? 'O2026.txt' : 'program_siemens.mpf';
    ...
    if (navigator.share && navigator.canShare) {
        try {
            await navigator.share({
                title: 'CNC Program',
                text: 'Код программы для станка ЧПУ:\n\n' + text.substring(0, 100) + '...', // Preview
                files: [file]
            });
            // Success
        } catch (err) {
            console.log('Share failed/cancelled', err);
            // Backup: just share text if file sharing fails
            try {
                await navigator.share({
                    title: 'CNC Program',
                    text: text
                ...
                showCustomAlert("Не удалось поделиться. Попробуйте скопировать текст.");
            }
        }
    } else {
        // Fallback for desktop/unsupported
        copyCode();
        showCustomAlert("Ваш браузер не поддерживает отправку. Текст скопирован в буфер.");
    }
}
...
async function saveFile() {
    ...
    if (currentMachine === 'fanuc' || currentMachine === 'fms' || currentMachine === 'haas') {
        // Ensure M30 and % at the end
        if (text.endsWith('%')) {
            text = text.substring(0, text.length - 1).trim();
        }

        if (!text.endsWith('M30')) {
            text += '\nM30';
        }
        text += '\n%';
    }
    ...
    if (!window.isSecureContext) {
        // If not secure, modern APIs like showSaveFilePicker are disabled.
        console.warn('Context is not secure. File Picker API disabled.');
    }

    try {
        // 1. Try Modern File System Access API (Desktop/Chrome Android - Secure Context Only)
        if (window.showSaveFilePicker) {
            const handle = await window.showSaveFilePicker({
                suggestedName: fileName,
                types: [{
                    description: 'CNC Program',
                    accept: { 'text/plain': ['.txt', '.mpf', '.nc', '.cnc'] }
                }],
            });
            ...
            setTimeout(() => {
                showCustomAlert("✅ Файл успешно сохранен на выбранный диск!");
            }, 100);
            return;
        }
    } catch (err) {
    ...
    setTimeout(() => {
        let msg = "✅ Файл скачан в папку 'Загрузки'!";
        if (!window.isSecureContext) {
            msg += "\n\n(Причина предупреждения: Вы тестируете через обычный HTTP. После загрузки на сервер (HTTPS) это предупреждение исчезнет и функции выбора диска заработают).";
        } else {
